...
### Custom vs Web search: cuándo cada una

No son alternativas — resuelven problemas distintos:

| Dimensión | Custom tool | Web search |
|-----------|-------------|------------|
| Qué define el dev | name, description, input_schema | Solo activar el toggle |
| Quién ejecuta | Tu código (después del `tool_use`) | Anthropic (server-side) |
| Para qué sirve | Acceder a tus sistemas (DB, CRM, API interna) | Info pública actualizada (precios, noticias, docs vigentes) |
| Fuente de la respuesta | Lo que devuelva tu función | Resultados web reales |
| Costo extra | Tokens del loop tool_use/tool_result | Tokens extra + costo de search |
| Reproducibilidad | Determinístico si tu función lo es | No — la web cambia |
| Riesgo de fabricar data | Bajo | Medio (el modelo puede resumir mal un resultado) |

La regla: **Custom para "mi negocio", Web search para "el mundo"**. Mezclalos libremente en el mismo request si la tarea cruza ambos dominios (ej: "buscá el precio actual de este producto y compará con nuestros precios de la DB").

### Conceptos de arquitecto

- **El Workbench es un validador del contrato del tool, no del tool en sí**. El modelo decide invocar en base al `description` + `input_schema`. Ese criterio de decisión es el 80% del trabajo de diseñar un tool; la implementación de la función real es el 20%.
- **Descriptions pobres son la causa #1 de "Claude no usa mi tool"**. Antes de debuggear tu código, probá el tool en el Workbench con 5-6 prompts distintos. Si el modelo no lo invoca cuando debería, el problema está en la description — no en tu código.
- **Web search no es Google**: no devuelve una lista de resultados para que vos elijas. Claude decide qué queries hace, lee los resultados, y los usa para responder. Podés ver las búsquedas en los content blocks `server_tool_use`, pero no controlás directamente qué busca.
- **Ningún tool es "free"**: cada round-trip `tool_use`/`tool_result` suma latencia (tu llamada + la respuesta) y tokens (el modelo relee el contexto). Si la tarea se puede resolver sin tool, resolvela sin tool.
...
- ❌ **Definir `required` vacío o `properties` sin descripciones en el schema**. El modelo genera argumentos mirando las descripciones de cada property. Un schema minimalista (`{"type":"object"}`) produce invocaciones con argumentos inventados o faltantes.
- ❌ **Probar tu tool solo con el caso feliz**. Un tool bien diseñado también se prueba con: (1) un prompt donde NO debería invocarse (¿Claude lo invoca de más?), (2) un prompt ambiguo (¿pide aclaración o inventa args?). El Workbench te deja validar estos tres casos en minutos.
- ❌ **Confiar en `tool_choice: any` para forzar una herramienta**. Forzar puede ocultar un bug de description: el modelo invoca el tool porque lo obligaste, no porque entendió cuándo hacerlo. Ese prompt va a fallar en producción donde no podés forzar cada vez.
